...
public class PDFUtil {
    private final static Logger log = LoggerFactory.getLogger(PDFUtil.class.getName());
    ...
    public static void main(String[] args) {
        ...
        createFileRequest.setSaveFileName("pdf.pdf");
        ...
        lineChartData2.setyAxis(Double.parseDouble("45"));
        ...
        lineChartData5.setyAxis(Double.parseDouble("50"));
    ...
    public static void createPDF(CreateFileRequest createFileRequest) {
        Configuration configuration = getConfiguration(createFileRequest.getTemplateFilePath());
        StringWriter stringWriter = new StringWriter();
        OutputStream outputStream = null;
        try {
            Template template = configuration.getTemplate(createFileRequest.getTemplateFileName());
            template.process(createFileRequest.getData(), stringWriter);
            stringWriter.flush();
            outputStream = new FileOutputStream(createFileRequest.getSaveFilePath() + File.separator + createFileRequest.getSaveFileName());
            getPDF(outputStream, stringWriter.toString());
        } catch (Exception ex) {
            log.error("创建pdf文件过程失败，失败原因：{}", ex.getMessage());
        } finally {
    ...
    private static void getPDF(OutputStream outputStream, String stringWriterStr) {
        Document document = new Document(PageSize.A4, 80, 80, 50, 50);
        try {
            PdfWriter pdfWriter = PdfWriter.getInstance(document, outputStream);
            document.open();
            XMLWorkerHelper helper = XMLWorkerHelper.getInstance();
            helper.parseXHtml(pdfWriter, document, new ByteArrayInputStream(stringWriterStr.getBytes()), Charset.forName(encode));
        } catch (Exception ex) {
            log.error("生成pdf文件失败，失败原因：{}", ex.getMessage());
        } finally {
            document.close();
        }
    }
